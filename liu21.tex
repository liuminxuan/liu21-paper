...
The \textit{Autonomous Vehicle Protocol for Merge Points} proposed by S. Aoki and R. Rajkumar is an interaction-based traffic protocol for safely cross a merge point, which is the intersection of 2 one-way lanes merging into one lane. 
...
The \textit{Autonomous Vehicle Protocol for Merge Points} \cite{10.1145/3055004.3055028} proposed by S. Aoki and R. Rajkumar is an interaction-based traffic protocol for safely cross a merge point, which is the intersection of 2 one-way lanes merging into one lane. 
...
However, we first revise the protocol to make our version distinguish from the original \textit{Autonomous Vehicle Protocol for Merge Points} in \cite{10.1145/3055004.3055028} by abstract some concepts.
...
where \verb!PREP! is a Maude variable of atomic propositions. 
The two equations mean that for all states $s$ $\in$ $S_{a-merging}L_{a-merging}(s)$ = {\verb!fin!} if and only if $s$ contains (\verb!gstat : fin!). 
...
We reduce the \verb!modelCheck(init, halt)! to check if the protocol enjoys the \textit{Starvation freedom} property.
It has no counterexample.
